...
void ThreadFinder::run()
{
    ...
    for (int i = 0; dirs.count() && toWork; ) // пока есть каталоги для поиска файла
    {
     ...
     {
      if (foundDirs[j].indexOf(file) != -1)
      {
       emit findFile(foundDirs[j]);
      }
     ...
     for (int j = 0; j < amount && toWork; j++) // пока не просмотрены все файлы или пока флаг
     // toWork не установлен в false
     {
